...
func (server *Server) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		responses.ERROR(w, http.StatusUnproccesableEntity, err)
	}
	user := models.User{}
	...
	userCreated, err := use.SaveUser(server.DB)
...
func (server *Server) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	body, err := ioutil.ReadAll(r.Body)
	if err != nil {
		reponses.ERROR(w, http.StatusUnprocessa)
		return
	}
	user := models.User{}
	err = json.Unmarshal(body, &user)
	...
	if tokenID != uint32(uid) {
		responses.ERROR(w, http.StatusUnauthorized, errors.new(http.StatusUnauthorized))
		return
	}
	user.Prepare()
